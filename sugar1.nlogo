globals [
  ;grow-back-delay
  ;barren-patch-percentage
  total-barren-patches
  total-sugar-harvested
]
...
to setup
  ...
  let total-psugar sum [psugar] of patches
  let average-psugar mean [psugar] of patches
  setup-patches
  set grow-back-delay 5
  reset-ticks
  set barren-patch-percentage 40
  set total-barren-patches 0
  set total-sugar-harvested 0
  setup-barren-patches
end
to setup-terrain
  ask patches [
    ;; assign a random terrain type (0, 1, or 2) to each patch
    set pcolor scale-color green random-float 1 2 0
  ]
end

to setup-barren-patches
  ask patches [
    if random-float 100 < barren-patch-percentage [
      set max-psugar 0
      set psugar 0
      set pcolor black
      set total-barren-patches total-barren-patches + 1
    ]
  ]
end
...
to go
  ...
  ask patches [
    if pcolor = black [
      set total-barren-patches total-barren-patches + 1 ; increment total-barren-patches
    ]
  ]
  ask turtles [
    ; move towards the patch with the most sugar
    let target-patch max-one-of patches with [pcolor != black and max-psugar > 0] [psugar]
    face target-patch
    ifelse distance target-patch > 1 [
      move-to target-patch
    ] [
      ; harvest sugar from the patch
      if [max-psugar] of target-patch > 0 [
        set sugar sugar + psugar
        set psugar 0
        if psugar <= 0 [
          set pcolor black
          set max-psugar 0
...
to-report average-patch-sugar
  let total-sugar sum [psugar] of patches
  let num-patches count patches with [psugar > 0]
  report total-sugar / num-patches
end
...
<experiments>
  <experiment name="experiment" repetitions="100" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="grow-back-delay">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="visualization">
      <value value="&quot;color-agents-by-vision&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="barren-patch-percentage">
      <value value="40"/>
    </enumeratedValueSet>
  ...
  <experiment name="exp1" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
    <metric>average-patch-sugar</metric>
    <metric>count turtles</metric>
    <metric>total-barren-patches</metric>
    <metric>total-sugar-harvested</metric>
    <enumeratedValueSet variable="grow-back-delay">
      <value value="2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="visualization">
